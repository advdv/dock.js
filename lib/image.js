...
var Image = function(){
  ...
  var args = arg([
    ...
    {from:         arg.OBJECT   | arg.Optional, _default: function(){ return false; }},
  ...
  self.build = function build() {
    ...
    //if from is false, normalize it using a promise cast
    var depBuild = _from().build;
    if(depBuild === undefined) {
      depBuild = function(){ return false; };
    }

    //build from image first, then tar, then build 
    self.built = Promise.cast(depBuild()).then(self.tar).then(function(tarFile){
